...
**Определение задачи:**  
Дан вектор вещественных чисел `v`. Необходимо вычислить количество пар элементов `(v[i], v[i+1])`, для которых `v[i] > v[i+1]`.

**Формат входных данных:**
- Вектор `v` длины `n` (n ≥ 0)
...
3. Проход по вектору:
   ```cpp
   for (size_t i = 0; i + 1 < v.size(); i += 1)
       if (v[i] > v[i+1] + eps) violations += 1;
...
1. Определяем количество процессов `p` и длину вектора `n`.
2. Базовое количество элементов на процесс: `base_chunk = n / p`.
3. Остаток: `remainder = n % p`.
...
**Граничные случаи:**
- Один процесс (`p=1`) → выполняется как последовательный алгоритм.
...
**Численная точность**
- Используется порог epsilon = 1e-10 для сравнения double
